...
export async function saveQuestionnaire(answers: Record<string, any>, topicId?: string) {
  try {
    ...
    // Build redirect URL with topicId if provided
    const summaryUrl = topicId
      ? `/onboarding/summary?topicId=${topicId}`
      : "/onboarding/summary";
